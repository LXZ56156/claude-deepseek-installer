...
# 发布验收（Full + build-release isolated output + simulate-user-release + sandbox 全模拟 + 场景矩阵）
...
验证层级：**Smoke（每次 commit）→ Full（每次 push）→ Release（发布前）→ Hardcore（专项验证）**。`All` 等于 `Full + Release + Hardcore`。

VM 验收层（独立于 `validate.ps1`，面向发布前真机/VM 全链路人工交互验收）：`scripts/vm-final-acceptance.ps1` 在专用 Win11 VMware 验收机内编排 Full→Release→Hardcore→ZIP 构建→从解压目录真实 `.cmd` 启动 ConPTY 提示驱动场景（16 TestSafe + 8 Live）。TestSafe 在 host 即可跑；Live 需管理员 + `C:\CCDI-ACCEPTANCE-VM.marker` + `-AcknowledgeRealInstall` + 首装无 claude/node/npm；只有真实重启续跑验收才额外传 `-AcknowledgeRestart`。详见 `docs/验收与交接体系.md` 第 11 章。AGENTS.md 显示 Dedicated VM Live 和真实重启仍未执行，未完成前不得 merge/tag/release。
...
| `scripts/vm-final-acceptance.ps1` | VM 内全链路人工交互验收入口（Full→Release→Hardcore→ZIP→ConPTY 场景）。TestSafe host 可跑；Live 需管理员 + `CCDI-ACCEPTANCE-VM.marker` + `-AcknowledgeRealInstall`；重启续跑另需 `-AcknowledgeRestart` |
...
| `scripts/data/interactive-acceptance-scenarios.json` | 24 个交互场景定义（16 TestSafe + 8 Live），每场景含 entry/timeoutSec/step.expect |
