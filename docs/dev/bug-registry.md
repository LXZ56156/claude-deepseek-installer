...
| ACC-037 | `interactive-user-acceptance.ps1` responder counter | TestSafe `install-skip-key` scenario exercises responder prompt matching without runner crash | source gate forbids `= (if` / `= (switch` in runner and specifically `$responderCounts[...] = (if ...)` | 否 |
