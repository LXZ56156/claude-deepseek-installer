...
**A:** 可能原因和解决方法：
1. Windows 安全提示被遮挡 → 看看任务栏有没有闪烁的图标
2. 被杀毒软件拦截 → 暂时关闭杀毒软件再试
3. 如果弹出 "Windows 保护了你的电脑" → 点击「更多信息」→「仍要运行」
4. 权限问题 → 右键「开始安装.cmd」→「以管理员身份运行」
5. 如果还是不行 → 截图窗口内容发给技术支持
...
### Q24: 如何生成诊断报告？
**A:** 双击「一键诊断.cmd」即可。报告保存在项目目录下，文件名为 `report-YYYYMMDD-HHMMSS.txt`。
