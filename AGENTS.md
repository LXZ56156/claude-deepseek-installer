...
- **Status**: VM acceptance final closure implemented on host: transactional resume registration/cleanup, strict SchemaVersion 3 prefix validation, UTF-8 ConPTY driver, static machine-state guard, transactional final summary, isolated TestSafe release output, redacted release API-key scan diagnostics, scenario/step final failureText scan, evidence-gated resume cleanup, release and Live command-collection StrictMode scalar Count fixes, Release Scenario B isolated mock Claude TestSafe fix, TestSafe API Key browser-open guard, ConPTY responder-count PowerShell 5.1 fix, whole-source parenthesized if/switch assignment gate, Scenario 4 API Key prompt regex fix, all sendSecret prompt anchoring guard, Live ownership optional-property and pre-stage cleanup fix, fallback install restart-dependency closure, deep-audit closure, release/repair closure through ACC-050, ACC-051 VM Live setup timeout closure, ACC-052 Live npm-missing required-text closure, ACC-053 Live anomaly-postcheck acceptance-contract closure, ACC-054..ACC-060 pre-release manual acceptance P0/P1 closure, and ACC-061 redacted backup rollback safety closure. Latest closure fixes: memory snapshots as internal failure rollback source (not redacted safe backups), Restore-SettingsJsonFromMemorySnapshot for all three config-write/restore paths, sk- key scan regex unified to `sk-[A-Za-z0-9_-]{20,}`. Source-side check, VM-acceptance functional (210 checks + 4 new ACC-061 checks), Smoke, Release, and Full pass in this session. Dedicated VM Live and real reboot remain pending. Do not merge/tag/release/upload until VM Live + real-reboot resume pass.
...
- **Latest fix**: ACC-061 redacted backup rollback safety — memory snapshots now serve as internal failure rollback source; safe backups remain on-disk only for after-sales/non-sensitive restore; release key-scan regex updated to `sk-[A-Za-z0-9_-]{20,}`; check/test-vm functional gates added.
...
| 日期 | 内容 | Commit |
|------|------|--------|
| 2026-06-26 | ACC-061 redacted backup rollback safety — memory snapshot/rollback, safe backup isolation, sk- key regex _ and - fix, 4 functional tests + check gates; check/Smoke/Release/Full pass | 本提交 |
| 2026-06-26 | ACC-054..060 pre-release manual acceptance P0/P1 closure — safe redacted settings backups, strict API Key validation, restore backup validation, Node/npm fresh-shell fallback readiness, repair-deps base/fallback distinction, Start-Here maintenance mode, and `ccdi-cfg-input` tracked-source gate; check/test-vm 210/Smoke/Release/Full pass | 本提交 |
| 2026-06-25 | ACC-053 Live anomaly-postcheck required-text closure — anomaly setup retained, required now only user-visible success, internal log text forbidden in required, failureText coverage retained; check/test-vm 194/Smoke/Release/Full pass | 本提交 |
| 2026-06-25 | ACC-052 Live npm-missing required-text closure — `live-npm-missing.required` matches current npm fallback output extracted from `claude-install.ps1`; check/test-vm 188 and Smoke pass | 本提交 |
| 2026-06-25 | ACC-051 VM Live setup timeout range closure — `Invoke-AcceptanceCapturedCommand` range raised to 1..1800; vm-final 900s call AST gate; fake-winget installNodeForFault tests for `live-npm-missing` and `live-install-command-anomaly-postcheck-usable`; check/test-vm 185/Smoke/Release/Full pass | 本提交 |
| 2026-06-25 | ACC-050 launcher pause/self-test closure — repair-launcher waits for finish prompt; generic .cmd pause drift gate; ConPTY final-clean uses Read-Host and writes result/stdout/stderr evidence; check/test-vm 173 and no-skip TestSafe doctor-mock-200 pass | 本提交 |
| 2026-06-25 | ACC-049 doctor mock launcher separation — mock API scenarios direct `doctor.ps1` PowerShell entry without `-SkipApiTest`; launcher skip behavior kept separate; check/test-vm 168/Smoke/Release/Full/target scenarios pass | 本提交 |
| 2026-06-24 | repair-deps Native PATH early-return closure (ACC-048) — Repair-ClaudePathIfNeeded before Claude-available early return; native fixed-path + User PATH missing/present/write-fail functional cases; fresh-shell verify; pathWriteFailed gating; check.ps1 1e gate | 本提交 |
| 2026-06-24 | Release Scenario C + repair-deps outcome closure — node.exe mock, Claude-first optional Node/npm, single final pause, 139 functional checks | 本提交 |
