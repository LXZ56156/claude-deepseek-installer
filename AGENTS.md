...
- **Status**: VM acceptance final closure implemented on host: transactional resume registration/cleanup, strict SchemaVersion 3 prefix validation, UTF-8 ConPTY driver, static machine-state guard, transactional final summary, isolated TestSafe release output, redacted release API-key scan diagnostics, scenario/step final failureText scan, evidence-gated resume cleanup, release and Live command-collection StrictMode scalar Count fixes, Release Scenario B isolated mock Claude TestSafe fix, TestSafe API Key browser-open guard, ConPTY responder-count PowerShell 5.1 fix, whole-source parenthesized if/switch assignment gate, Scenario 4 API Key prompt regex fix, all sendSecret prompt anchoring guard, Live ownership optional-property and pre-stage cleanup fix, fallback install restart-dependency closure, deep-audit closure, release/repair closure through ACC-050, and ACC-051 VM Live setup timeout closure. The latest VM Live failure was Scenario 20 `live-npm-missing` at `Start-LiveScenarioSetup`: `Invoke-AcceptanceCapturedCommand -TimeoutSec 900` exceeded the old `[ValidateRange(1, 300)]`, also affecting `live-install-command-anomaly-postcheck-usable`. Source-side fix is complete: captured-command timeout range is `1..1800`, check.ps1 AST-scans vm-final captured-command timeout values against the helper range, and `test-vm-acceptance.ps1` proves `-TimeoutSec 900` binding plus fake-winget `installNodeForFault` setup for both affected Live scenarios without real install. Source-side check, VM-acceptance functional (185 checks), Smoke, Release, and Full pass in this session. Current non-dedicated host `vm-final -Mode TestSafe` was previously blocked before scenarios by active `.claude`/node noise; previous isolated TestSafe interactive scenarios pass. Dedicated VM Live and real reboot remain pending. Because the failed Live run already reached Scenario 20 setup, clean the VM before rerunning Live 8. Do not merge/tag/release/upload until VM Live + real-reboot resume pass.
