...
- **Status**: VM acceptance final closure implemented on host: transactional resume registration/cleanup, strict SchemaVersion 3 prefix validation, UTF-8 ConPTY driver, static machine-state guard, transactional final summary, isolated TestSafe release output, redacted release API-key scan diagnostics, scenario/step final failureText scan, evidence-gated resume cleanup, release and Live command-collection StrictMode scalar Count fixes, Release Scenario B isolated mock Claude TestSafe fix, TestSafe API Key browser-open guard, ConPTY responder-count PowerShell 5.1 fix, whole-source parenthesized if/switch assignment gate, Scenario 4 API Key prompt regex fix, all sendSecret prompt anchoring guard, Live ownership optional-property and pre-stage cleanup fix, fallback install restart-dependency closure, deep-audit closure, and current release/repair closure: Scenario C release simulation uses `node.exe` fixed-path mock evidence, `repair-deps.ps1` is Claude-first/outcome-first with optional Node/npm when Claude is usable, and `一键修复依赖.cmd` owns the single final pause through `-NoFinalPause`. `repair-deps.ps1` also repairs Native Install PATH (User PATH) before the Claude-available early return when Claude is usable via the `.local\bin` fixed path (ACC-048), with TestSafe/mock native fixed-path + User PATH missing/present/write-fail functional cases and a check.ps1 1e gate. ACC-049 source-side fix is complete after tests: `doctor-mock-*` scenarios run `doctor.ps1 -ShareSafe -NoOpenReport` via safe `entryMode="powershell"` without `-SkipApiTest`, while `diagnostic-launcher` keeps proving `一键诊断.cmd` skips API by default. ACC-050 source-side fix is complete after tests: `repair-launcher` now waits for `Press any key to finish`, check.ps1 has a generic `.cmd` pause-drift gate, and the ConPTY driver self-test final-clean race is fixed by using readiness-coupled `Read-Host` plus transcript/stdout/stderr/result evidence. Source-side check, VM-acceptance functional (173 checks), no-skip TestSafe `doctor-mock-200`, no-skip TestSafe `repair-launcher`, Smoke, Release, and Full pass in this session; the no-skip runs prove the default driver self-test path no longer hangs. Current non-dedicated host `vm-final -Mode TestSafe` is blocked before scenarios by active `.claude`/node noise; previous isolated TestSafe interactive scenarios pass. Dedicated VM Live and real reboot remain pending. Do not merge/tag/release/upload until VM Live + real-reboot resume pass.
